...
- `vm.cpp` The VM shell simulates virtual memory operations. init ps vp fr tlb [CLOCK|LRU] sets up the VM with page size ps, virtual pages vp, physical frames fr, TLB size tlb, and replacement policy. read a reads a byte from address a, and write a b writes value b (0–255) to address a. fill a len b writes value b to len bytes starting at a. seq start ops [w] and rand ops max [seed] [w] perform sequential or random accesses, optionally writing. dumpvpn v and dumpf f show virtual page or frame info, stats reports memory and TLB stats, policy P changes replacement policy, reset reinitializes the VM, and back exits. For example, init 256 16 8 4 LRU creates 16 pages of 256 bytes, 8 frames, a 4-entry TLB, and LRU replacement; write 0 42 writes 42 to address 0, write 300 84 writes 84 to 300, and read 0 reads back 42.
